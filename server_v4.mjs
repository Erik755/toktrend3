...
app.get('/health', async (req, res) => {
  ...
  res.json({ ok: true, version: "gtts-v4-opt-14", tiktokConnected: Boolean(token), tokenValid: tokenValid, tokenExpiresAt: tokenExpiry ? new Date(tokenExpiry * 1000).toISOString() : null, time: new Date().toISOString(), logs: appLogs });
});
...
async function buildVideo(images, audioPath, outputPath, totalSeconds = 29) {
  ...
  const cmd = [
    ...
    `-c:v libx264 -preset faster -crf 23 -b:v 1800k -maxrate 2500k -bufsize 5000k -threads 2`,
