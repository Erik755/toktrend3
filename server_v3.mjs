...
app.get('/health', async (req, res) => {
  ...
  res.json({ ok: true, version: "gtts-v3-hq-13", tiktokConnected: Boolean(token), tokenValid: tokenValid, tokenExpiresAt: tokenExpiry ? new Date(tokenExpiry * 1000).toISOString() : null, time: new Date().toISOString(), logs: appLogs });
});
...
async function buildVideo(images, audioPath, outputPath, totalSeconds = 29) {
  ...
  const cmd = [
    ...
    `-c:v libx264 -preset medium -crf 18 -b:v 2500k -maxrate 4000k -bufsize 8000k`,
