...
app.get('/health', async (req, res) => {
  ...
  res.json({ ok: true, version: "gtts-v6-kenbr-16", tiktokConnected: Boolean(token), tokenValid: tokenValid, tokenExpiresAt: tokenExpiry ? new Date(tokenExpiry * 1000).toISOString() : null, time: new Date().toISOString(), logs: appLogs });
});
...
async function buildVideo(images, audioPath, outputPath, totalSeconds = 29) {
  ...
  for (let i = 0; i < images.length; i++) {
    ...
    const segCmd = `"${ffmpegPath.path}" -y -loop 1 -i "${imgPath}" -t ${(framesPerImg/fps).toFixed(3)} -vf "${vf}" -c:v libx264 -preset faster -b:v 2000k -maxrate 3000k -bufsize 6000k -an "${tmpOut}"`;
