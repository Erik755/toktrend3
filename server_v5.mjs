...
app.get('/health', async (req, res) => {
  ...
  res.json({ ok: true, version: "gtts-v5-cbr-15", tiktokConnected: Boolean(token), tokenValid: tokenValid, tokenExpiresAt: tokenExpiry ? new Date(tokenExpiry * 1000).toISOString() : null, time: new Date().toISOString(), logs: appLogs });
});
...
async function buildVideo(images, audioPath, outputPath, totalSeconds = 29) {
  ...
  const cmd = [
    ...
    `-c:v libx264 -preset faster -b:v 2000k -minrate 1000k -maxrate 3000k -bufsize 6000k -threads 2`,
  ...
  console.log('[ffmpeg] Video:', outputPath);
}
