...
app.get('/health', async (req, res) => {
  ...
  res.json({ ok: true, version: "gtts-v7-kf15-17", tiktokConnected: Boolean(token), tokenValid: tokenValid, tokenExpiresAt: tokenExpiry ? new Date(tokenExpiry * 1000).toISOString() : null, time: new Date().toISOString(), logs: appLogs });
});
...
async function buildVideo(images, audioPath, outputPath, totalSeconds = 29) {
  // Ken Burns effect: zoom suave por imagen para generar motion y bitrate adecuado para TikTok
  const fps = 30;
  const framesPerImg = Math.floor((totalSeconds * fps) / images.length);
  const tempVideos = [];

  for (let i = 0; i < images.length; i++) {
    const imgPath = images[i];
    const tmpOut = outputPath + `.seg${i}.mp4`;
    // Alternar zoom-in y zoom-out para variedad visual
    const zoomIn = i % 2 === 0;
    const zoompan = zoomIn
      ? `zoompan=z='min(zoom+0.0008,1.3)':d=${framesPerImg}:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)'`
      : `zoompan=z='if(lte(zoom,1.0),1.3,max(1.001,zoom-0.0008))':d=${framesPerImg}:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)'`;
    const vf = `scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,${zoompan},scale=720:1280,setsar=1,fps=${fps}`;
    const segCmd = `"${ffmpegPath.path}" -y -loop 1 -i "${imgPath}" -t ${(framesPerImg/fps).toFixed(3)} -vf "${vf}" -c:v libx264 -preset fast -b:v 2500k -minrate 2000k -maxrate 4000k -bufsize 8000k -g 15 -bf 0 -an "${tmpOut}"`;
    await execAsync(segCmd, { maxBuffer: 200 * 1024 * 1024 });
    tempVideos.push(tmpOut);
  }

  // Concatenar segmentos de video
  const concatFile = outputPath + '.concat.txt';
  fs.writeFileSync(concatFile, tempVideos.map(f => `file '${f}'`).join('\n'));
  const finalCmd = `"${ffmpegPath.path}" -y -f concat -safe 0 -i "${concatFile}" -i "${audioPath}" -c:v libx264 -preset faster -b:v 2000k -maxrate 3000k -bufsize 6000k -c:a aac -b:a 128k -ar 44100 -ac 2 -shortest -movflags +faststart -t ${totalSeconds} "${outputPath}"`;
  await execAsync(finalCmd, { maxBuffer: 200 * 1024 * 1024 });

  tempVideos.forEach(f => { try { fs.unlinkSync(f); } catch {} });
  try { fs.unlinkSync(concatFile); } catch {}
  console.log('[ffmpeg] Video HQ Ken Burns:', outputPath);
}
...
app.post('/api/publish', async (req, res) => {
  try {
    ...
    const videoUrl = `${protocol}://${req.headers.host}/videos/${sessionId}/video.mp4`;
    console.log('[Publish] URL:', videoUrl);
    ...
    console.log('[Publish] TikTok:', JSON.stringify(response.data));
    if (response.data.error?.code && response.data.error.code !== 'ok') throw new Error(JSON.stringify(response.data.error));
